...
// overrideFileUploadCommands replaces generated upload-file and complete-file-upload
// with manual implementations in the file-uploads group.
func overrideFileUploadCommands(group *cobra.Command) {
	// Remove generated upload-file and complete-file-upload
	toRemove := []string{"upload-file", "complete-file-upload"}
	remaining := []*cobra.Command{}
	...
	// Cobra has no RemoveCommand — rebuild
	for _, sub := range group.Commands() {
		group.RemoveCommand(sub)
	}
